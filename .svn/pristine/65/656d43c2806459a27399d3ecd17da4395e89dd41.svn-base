...
class Pointgoods extends IndexHome
{
    ...
    */
    public function index()
    {
        ...
        $order = 'po.o_id asc';
        $field='po.o_id,po.o_point,po.o_periods,po.o_addtime,prg.rg_content,pro.add_time as ro_add_time,prg.rg_id,po.gp_num,prg.gp_id,m.m_id,m.m_name,m.m_avatar,prg.goods_score';
        $limit = "2";
...
//        $tui_user = Db::connect('db_syu')->name('member_content')->where($where)->field('mc_id,m_name,m_avatar,tui_diamond')->find();
        $tui_user['tui_diamond'] = Db::connect('db_syu')->name('member_content')->where($where)->value('tui_diamond');
        $find = Db::table('pai_member')->where($where)->field('m_name,m_avatar')->find();
