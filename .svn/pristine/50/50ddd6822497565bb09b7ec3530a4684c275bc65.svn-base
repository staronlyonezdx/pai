...
namespace app\data\service\goodsImgs;
use app\data\model\goodsCategory;
use app\data\model\goodsImgs\GoodsImgsModel;
use app\data\model\GoodsProduct\GoodsProductModel;
use app\data\service\BaseService as BaseService;
use app\data\service\popularity\PopularityGoodsService;
use think\Db;
class GoodsImgsService extends BaseService
{
	protected $cache = 'goods_imgs';
	
	public function __construct()
	{
		parent::__construct();
        $this->goodsImgs  = new GoodsImgsModel();

		$this->cache = 'goods_imgs';
	}
    ...
    public function del_goods_img($data)
    {
        ...
        if($imgs[0]['gi_id'] == $data['gi_id']){   //如果删除的是第一个(第一个为主图),默认添加更新第二个位主图
            if(!empty($imgs[1])){
                Db::table("pai_goods")->where('g_id',$data['g_id'])->update(['g_img'=>$imgs[1]['gi_name']]);
                Db::table("pai_goods_product")->where('g_id',$data['g_id'])->update(['gp_img'=>$imgs[1]['gi_name']]);
            }else{
                Db::table("pai_goods")->where('g_id',$data['g_id'])->update(['g_img'=>'']);
                Db::table("pai_goods_product")->where('g_id',$data['g_id'])->update(['gp_img'=>'']);
            }
        }
        $where = [
            'gi_id'=>$data['gi_id'],
