...
//            'g.g_state'=>['not in',[1,7]],
        ...
        foreach($gdr_list as $k => $v){
            ...
            $orderPaiList = $orderPai ->get_json_member($order_where,'o.gdr_id,o.o_id,o.gp_num,o.o_state,o.o_periods,o.o_periods,o.gdr_id,o.gdr_id,o.o_addtime,m.m_id,m.m_name,m.m_avatar','o.o_id asc','');
        ...
        if($tj_list){
            ...
            $tj_list = $goods->get_tj_list($where11,$order='g.g_id desc',$field="g.g_id,g.g_name,g.g_img,g.gc_id");
            $redis -> set('goods_promoters_tj_list'.$g_id,json_encode($tj_list),6);
        }
        //足迹信息
        if($m_id){
            //商品收藏(登录时查询是否已经收藏)
...
//        dump($list);die;
    ...
    public function order_comment($gp_id,$page,$page_size){
        ...
        $field='op.o_id,op.gp_id,op.o_paytime,op.gdr_id,op.o_periods,op.gp_num,op.o_publishtime,ro.service_score,ro.logistics_score,rg.rg_showname,rg.goods_score,rg.rg_id,rg.rg_content,m.m_name,m.m_avatar,ro.add_time,gdt.gdt_name';
