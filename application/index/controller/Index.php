...
class Index extends IndexHome
{
    public function add_phone(){
        set_time_limit(0);
        $data1 = file_get_contents('11.txt');
        $data2 = file_get_contents('22.txt');
        $data1 = explode(',',$data1);
        $data2 = explode(',',$data2);
...
//            ];
            ...
            $is_null = Db::table('pai_member')->where($where)->count();
            if($is_null){
                Db::table('pai_member')->where($where)->update(['is_jq'=>1]);
            }
        }
        foreach($data2 as $v){
//            $str = 'm'.substr($v,-5,5);
...
//            ];
    ...
    public function index()
    {
        ...
        if($imgs){
            ...
            $redis->set('imgs',json_encode($imgs),600);
        }
        //热拍
        //$heat = $this->goods_hot();
        //搜索自己的历史
...
//            'heat'          =>$heat,
        ...
        if(request()->isAjax()){
            ...
            if($page == 1){
                ...
                if($heat){
                    ...
                    $redis->set('first_heat',json_encode($heat),300);
                }
            }else{
                $heat = $goods->get_heat_list('g.is_heat desc',$page,$page_size);
            }
     ...
     * 邓赛赛
     */
    public function ajax_top_two_list(){
        $page = input('param.page',1);
