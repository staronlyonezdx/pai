...
{
	#app基本信息
	protected $sdkappid = 1400143371;
	protected $usersig = 'eJxlj0FPgzAAhe-8CsJVoy2lMJfssG5OzSYbwyW4C*lGwUJsm1LGiPG-G3HJSHzX78t7eV*WbdvO2yq*o8ejbIRJTaeYY49tBzi3V6gUz1JqUqSzf5CdFdcspblhuocQY*wCMHR4xoThOb8YivKOdnQg1FmV9it-DR4A0EMogEOFFz18fYxmL8*eXqKPXZHk23Ld5Kt2XrF2uRU3koUHEJOmLatNuC8OcTTlRPrJPIBkt6439-6JvMt9iCK9wE8lTBakEqAShQjOoxmZTiaDScM-2eWS58IRfnD9AT0xXXMpesEFEEMXgd841rf1AzBhXcw_';
	protected $identifier = 'paiyaya';
	...
	public function openim_send_msg($account_id, $receiver, $content)
	{
		$desc = json_decode($content,true);
		$end_desc = !empty($desc['share_content']) ? $desc['share_content'] : "";
		#构造高级接口所需参数
		$msg_content = array();
		//创建array 所需元素
	...
	{
		$title = !empty($msg_content[0]['MsgContent']['Desc']) ? $msg_content[0]['MsgContent']['Desc'] : "";//推送的标题
		$data = !empty($msg_content[0]['MsgContent']['Data']) ? $msg_content[0]['MsgContent']['Data'] : "";
		if(!empty($data)){
			$share_data = json_decode($data,true);
		}
		$end_desc = !empty($share_data['share_content']) ? $share_data['share_content'] : "";//这是离线推送的内容
		#构造新消息 
		$msg = array(
				'To_Account' => $receiver,
				'MsgSeq' => rand(1, 65535),
		...
		#将消息序列化为json串
		$req_data = json_encode($msg);
		$ret = $this->api("openim", "sendmsg", $this->identifier, $this->usersig, $req_data,false);
		$ret = json_decode($ret, true);
		return $ret;
	}
	...
	{
		$desc = json_decode($text_content,true);
		$end_desc = !empty($desc['share_content']) ? $desc['share_content'] : "";
		#构造高级接口所需参数
		$msg_content = array();
		//创建array 所需元素
	...
	public function openim_batch_sendmsg2($account_list, $msg_content)
	{
		...
		$ret = $this->api("openim", "batchsendmsg", $this->identifier, $this->usersig, $req_data,false);
        $ret = json_decode($ret, true);
		return $ret;
	}
	...
	public function profile_portrait_get($account_id)
	{
		...
		$tag_list = array(
				...
				"Tag_Profile_IM_AllowType",//加好友状态
	...
	public function profile_portrait_set3($account_id, $arr)
	{
//		print_r($arr);die();
		#构造高级接口所需参数
		$profile_list = array();
		...
		array_push($profile_list, $profile_grade);

		$ret = $this->profile_portrait_set2($account_id, $profile_list);
		$result = $this->profile_portrait_set2($account_id, $profile_grade);
		return $ret;
	}
	...
	public function profile_portrait_set2($account_id, $profile_list)
	{
		...
		$ret = $this->api("profile", "portrait_set", $this->identifier, $this->usersig, $req_data,false);
		$ret = json_decode($ret, true);
		return $ret;
	}
