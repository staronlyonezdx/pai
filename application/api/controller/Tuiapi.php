 ...
 * 2018-9-3
 */
define("PAIYY_URL","https://m.paiyy.com.cn");
class Tuiapi extends Controller
{
    /* * ******************************** 前台control父类 ********************************************* */
    //客户端类型
    ...
    public function __construct() {
        ...
        if(!defined("CDN_URL")){
            define("CDN_URL","https://m.paiyy.com.cn");//2018-8-31修改
        }
    ...
    public function get_member_info() {
        ...
        //解密
        $baserver = new BaseService();
//        $this->response_error($this->data);
        $member_id = empty($this->data['member_id']) ? '' : $this->data['member_id'] ;
        if(empty($member_id)){
            $member_id = empty($this->data['m_id']) ? '' : $this->data['m_id'] ;
        }
        if(!empty($member_id)){
            $m_id = $baserver->decrypt($member_id);
            $this->data['m_id'] = str_replace('abcde','',$m_id);//删除多余字符(加盐字符串)
            $this->data['member_id'] = str_replace('abcde','',$m_id);//删除多余字符(加盐字符串)
        }
        if (isset($this->data['member_id']) && isset($this->data['phone'])) {
            $m_mobile  = empty($this->data['phone']) ? '' : $this->data['phone'] ;
            if (empty($member_id)) {
                //如果需要检查是否是登录
                if ($this->check_login) {
                    $this->response_error('参数错误');
