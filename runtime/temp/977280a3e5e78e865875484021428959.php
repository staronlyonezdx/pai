<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:77:"D:\project\pai\public/../application/business/view/pointgoods/goods_list.html";i:1541491289;s:67:"D:\project\pai\public/../application/business/view/common/base.html";i:1541491289;s:69:"D:\project\pai\public/../application/business/view/common/header.html";i:1541491289;s:67:"D:\project\pai\public/../application/business/view/common/menu.html";i:1543453232;s:69:"D:\project\pai\public/../application/business/view/common/footer.html";i:1542013166;}*/ ?>
<!--_meta 作为公共模版分离出去-->
<!DOCTYPE HTML>
<html>
<head>
...
<aside class="Hui-aside">
    <div class="menu_dropdown bk_2">
        <dl id="menu-article">
            ...
            <dd>
                <ul>
                    <li class="itemli"><a href="/business/goods/goods_list" title="余额商品列表">1.余额商品列表</a></li>
                    <li class="itemli"><a href="/business/goods/add_goods" title="添加余额商品">添加余额商品</a></li>
    ...
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 产品管理 <span class="c-gray en">&gt;</span> 产品列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
        <div style="margin-left:50px;">
            <div class="pd-20">
                ...
                <div>
                    <span>商品状态：</span>
                    <a href="/business/pointgoods/goods_list/g_state/0" type="button" class="btn <?php if($g_state == '0'): ?>btn-success <?php else: ?> btn-info <?php endif; ?> btn-xs">全部</a>
                    <a href="/business/pointgoods/goods_list/g_state/1" type="button" class="btn <?php if($g_state == '1'): ?>btn-success <?php else: ?> btn-info <?php endif; ?> btn-xs">竞拍中</a>
                    <a href="/business/pointgoods/goods_list/g_state/2" type="button" class="btn <?php if($g_state == '2'): ?>btn-success <?php else: ?> btn-info <?php endif; ?> btn-xs">草稿</a>
                    <a href="/business/pointgoods/goods_list/g_state/3" type="button" class="btn <?php if($g_state == '3'): ?>btn-success <?php else: ?> btn-info <?php endif; ?> btn-xs">审核中</a>
                    <a href="/business/pointgoods/goods_list/g_state/4" type="button" class="btn <?php if($g_state == '4'): ?>btn-success <?php else: ?> btn-info <?php endif; ?> btn-xs">已失败</a>
                </div>
                <div class="mt-20">
                    <table class="table table-border table-bordered table-bg table-hover table-sort">
                        ...
                        <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                            <tr class="text-c va-m tr_<?php echo $vo['g_id']; ?>">
                                ...
                                <td class="is_heat<?php echo $vo['g_id']; ?>"><?php if($vo['is_heat']==0): ?>非热拍<?php else: ?><?php echo $vo['is_heat']; endif; ?></td>
                                ...
                                <td><?php switch($vo['g_state']): case "1": ?>未支付保证金<?php break; case "2": ?>审核中<?php break; case "4": ?>已失败<?php break; case "6": ?>竞拍中<?php break; case "7": ?>编辑中<?php break; case "8": ?>已流拍<?php break; endswitch; ?></td>
                                <td>
                                    <?php if($vo['is_heat'] > 0): ?>
                                    ...
                                    <?php endif; switch($vo['g_state']): case "1": ?><button type="button" class="btn btn-warning btn-xs" onclick="cancel('<?php echo $vo['g_id']; ?>')">下架</button><?php break; case "2": ?><button type="button" class="btn btn-warning btn-xs" onclick="cancel('<?php echo $vo['g_id']; ?>')">下架</button>
                                                         <a href="/business/pointgoods/reedit/g_id/<?php echo $vo['g_id']; ?>"  type="button" class="btn btn-success btn-xs">再次编辑</a>
                                        <?php break; case "4": ?><button type="button" class="btn btn-warning btn-xs" onclick="cancel('<?php echo $vo['g_id']; ?>')">下架</button>
                                                        <a href="/business/pointgoods/reedit/g_id/<?php echo $vo['g_id']; ?>" type="button" class="btn btn-info btn-xs">重新编辑</a>
                                        <?php break; case "6": ?><button type="button" class="btn btn-primary btn-xs" onclick="gp_stock('<?php echo $vo['g_id']; ?>','<?php echo $vo['gp_stock']; ?>')">修改库存</button>
                                                        <button type="button" class="btn btn-primary btn-xs" onclick="is_heat('<?php echo $vo['g_id']; ?>','<?php echo $vo['is_heat']; ?>')">热拍</button>
                                                        <button type="button" class="btn btn-primary btn-xs" onclick="is_top('<?php echo $vo['g_id']; ?>','<?php echo $vo['is_top']; ?>')">顶置</button>
                                                        <button type="button" class="btn btn-primary btn-xs" onclick="is_tj('<?php echo $vo['g_id']; ?>','<?php echo $vo['is_tj']; ?>')">推荐</button><?php break; case "7": ?><a href="/business/pointgoods/reedit/g_id/<?php echo $vo['g_id']; ?>"  type="button" class="btn btn-info btn-xs">继续编辑</a>
                                                        <button type="button" class="btn btn-warning btn-xs" onclick="cancel('<?php echo $vo['g_id']; ?>')">下架</button>
                                        <?php break; case "8": ?><a href="/business/pointgoods/reedit/g_id/<?php echo $vo['g_id']; ?>"  type="button" class="btn btn-success btn-xs">再次编辑</a><?php break; endswitch; ?>
                                </td>
                            </tr>
...
<script type="text/javascript">
    ...
    function cancel(g_id){
        $.post("/business/pointgoods/cancel",{g_id:g_id},function(res){
            if(res.status == 1){
                $('.tr_'+g_id).remove();
    ...
    function gp_stock(g_id,val){
        layer.prompt({title: '请输入库存数量',value:val, formType: 0}, function(text){
            ...
            $.post("/business/pointgoods/set_stock",{g_id:g_id,gp_stock:text},function(res){
                if(res.status == 1){
    ...
    function is_heat(g_id,val){
        layer.prompt({title: '设置热拍从小到大排序',value:val, formType: 0}, function(text){
            ...
            $.post("/business/pointgoods/set_goods",{g_id:g_id,is_heat:text},function(res){
                if(res.status == 1){
                    if(text != 0){
    ...
    function is_top(g_id,val){
        layer.prompt({title: '设置置顶从小到大排序',value:val, formType: 0}, function(text){
            ...
            $.post("/business/pointgoods/set_goods",{g_id:g_id,is_top:text},function(res){
                if(res.status == 1){
                    if(text != 0){
    ...
    function is_tj(g_id,val){
        layer.prompt({title: '设置热拍从小到大排序',value:val, formType: 0}, function(text){
            ...
            $.post("/business/pointgoods/set_goods",{g_id:g_id,is_tj:text},function(res){
                if(res.status == 1){
                    if(text != 1){
    ...
    function revoke_heat(g_id){
        var self = $(this)
        $.post("/business/pointgoods/set_goods",{g_id:g_id,is_heat:0},function(res){
            if(res.status == 1){
