<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:68:"D:\project\pai\public/../application/admin/view/adminauth/index.html";i:1541491286;s:64:"D:\project\pai\public/../application/admin/view/common/base.html";i:1543283930;}*/ ?>
	...
	<div class="container-fluid">
        <div class="panel panel-default">
            <div class="panel-body">
                ...
                <div class="form-group  from_add">
                    <a href="<?php echo url('Adminauth/authedit'); ?>">新增权限</a>
                </div>

                <?php if(empty($authorityList) || (($authorityList instanceof \think\Collection || $authorityList instanceof \think\Paginator ) && $authorityList->isEmpty())): ?>
                    <div class="form-group form_part">没有数据</div>
                <?php else: if(is_array($authorityList) || $authorityList instanceof \think\Collection || $authorityList instanceof \think\Paginator): $i = 0; $__LIST__ = $authorityList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <div class="form-group form_part">
                        <label><?php echo $vo['authority_name']; ?></label>
                        &nbsp;
                        <a href="<?php echo url('Adminauth/authedit',['authority_id' => $vo['authority_id']]); ?>"> 编辑</a>
                    </div>
                    <?php if(!(empty($vo['child_auth']) || (($vo['child_auth'] instanceof \think\Collection || $vo['child_auth'] instanceof \think\Paginator ) && $vo['child_auth']->isEmpty()))): ?>
                        <div class="form-group form-list-span">
                        <?php if(is_array($vo['child_auth']) || $vo['child_auth'] instanceof \think\Collection || $vo['child_auth'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['child_auth'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): $mod = ($i % 2 );++$i;?>
                        <span>
                            <label><?php echo $child['authority_name']; ?></label>
                            &nbsp;
                            <a href="<?php echo url('Adminauth/authedit',['authority_id' => $child['authority_id']]); ?>"> 编辑</a>
                            |
                            <a href="<?php echo url('Adminauth/authdel',['authority_id' => $child['authority_id']]); ?>" class="authoritylist_dele_btn">删除</a>
                        </span>
                        <?php endforeach; endif; else: echo "" ;endif; ?>
                        </div>
